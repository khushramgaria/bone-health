...
for i, img_name in enumerate(fracture_images, 1):
    try:
        ...
        # Predictions
        siglip_prediction = "FRACTURE" if siglip_prob > 0.5 else "NO FRACTURE"
        vit_prediction = "FRACTURE" if vit_prob > 0.5 else "NO FRACTURE"
        ensemble_prediction = "FRACTURE" if ensemble_prob > 0.5 else "NO FRACTURE"
        
        # Correctness
        siglip_correct = "✓" if siglip_prob > 0.5 else "✗"
        vit_correct = "✓" if vit_prob > 0.5 else "✗"
        ensemble_correct = "✓" if ensemble_prob > 0.5 else "✗"
        ...
        # Detailed log
        print(f"[{i:3d}/{len(fracture_images)}] {img_name:45s}")
        print(f"         True Label: FRACTURE")
        print(f"         SigLIP:     {siglip_prediction:12s} (Conf: {siglip_prob*100:5.1f}%) {siglip_correct}")
        print(f"         ViT:        {vit_prediction:12s} (Conf: {vit_prob*100:5.1f}%) {vit_correct}")
        print(f"         Ensemble:   {ensemble_prediction:12s} (Conf: {ensemble_prob*100:5.1f}%) {ensemble_correct}")
        print()
            
    except Exception as e:
        print(f"[{i:3d}/{len(fracture_images)}] ❌ {img_name:45s} | Error: {str(e)}\n")
...
for i, img_name in enumerate(no_fracture_images, 1):
    try:
        ...
        # Correctness (correct if predicted NO FRACTURE)
        siglip_correct = "✓" if siglip_prob <= 0.5 else "✗"
        vit_correct = "✓" if vit_prob <= 0.5 else "✗"
        ensemble_correct = "✓" if ensemble_prob <= 0.5 else "✗"
        ...
        # Detailed log
        print(f"[{i:3d}/{len(no_fracture_images)}] {img_name:45s}")
        print(f"         True Label: NO FRACTURE")
        print(f"         SigLIP:     {siglip_prediction:12s} (Conf: {siglip_prob*100:5.1f}%) {siglip_correct}")
        print(f"         ViT:        {vit_prediction:12s} (Conf: {vit_prob*100:5.1f}%) {vit_correct}")
        print(f"         Ensemble:   {ensemble_prediction:12s} (Conf: {ensemble_prob*100:5.1f}%) {ensemble_correct}")
        print()
        ...
        print(f"[{i:3d}/{len(no_fracture_images)}] ❌ {img_name:45s} | Error: {str(e)}\n")


print(f"\n{'='*100}")
print(f"✓ TOTAL IMAGES PROCESSED: {len(y_true)}")
print(f"  - Fracture images:    {len(fracture_images)}")
print(f"  - No fracture images: {len(no_fracture_images)}")
print(f"{'='*100}")
